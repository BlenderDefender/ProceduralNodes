...
class PROCEDURALNODES_MT_main_menu(Menu):
    ...
    def draw(self, context):
        ...
        dirpath = node_path(context)
        if dirpath == "":
            layout.label(text="Your Operating system is not supported yet. Please open a OS-Request on GitHub.")
            return
    ...
    def execute(self, context):
        ...
        if extension == ".blend":
            with bpy.data.libraries.load(source) as (data_from, data_to):
                ...
                texts = [name for name in data_from.texts if name.endswith("f")]
        ...
        if dirpath != "" and extension == ".blend" and verified == "True":
            ...
            self.report({'WARNING'}, "Your Operating system is not supported yet. Please open a OS-Request on GitHub.")
